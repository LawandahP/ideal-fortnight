...
const ManualPaymentForm = ({invoice}) => {

    const dispatch = useDispatch()


    const createPayment = useSelector(state => state.createPayment)
    const { loading, error, success, message } = createPayment
    ...
    const submitHandler = (e) => {
        e.preventDefault()
        if (validate()) {
            dispatch(createPaymentAction(values, invoice?.invoice_id));
        } 
    }
                    ...
                    <Grid container spacing={2}>
                        <Grid item md={6} sm={6} xs={12}>                          
                            <Controls.InputField
                                ...
                                error={
                                    invoice.balance < 1 && values.amount_paid > invoice.total_amount 
                                    ? "Amount is greater than total invoice amount." 
                                    : invoice.balance > 1 && values.amount_paid > invoice.balance
